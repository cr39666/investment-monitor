...
- **表头多功能切换**：表头列可在多种指标间切换；支持**列自定义**设置，可在设置中拆分/合并涨跌幅、总盈比、持仓市值等列
...
- **设置项分类归集**，结构更清晰
  - **双语支持**（中 / 英 / 混搭），**开机自启**，**热键录制**：自定义全局展开 / 收起快捷键
  - **模块显示**：A 股 / 基金 / 黄金 三模块自定义选择开启
  - **列表列自定义**：可自定义列表列，支持拆分/合并部分列，调整列顺序
- **关于界面 & 版本更新**
  - 自动检测版本更新 && 手动检测更新
  - 提供 📒 入口直达 Releases 查看完整版本更新日志

### 🪟 无边框沉浸窗口

- 完全 **无边框透明** 窗口，圆角暗色主题，极致沉浸
- 各类输入与确认弹窗均支持 **Enter 确认 / ESC 取消**
...
│           │   ├── ColumnOrderModal.vue  # 表格列自定义弹窗
│           │   ├── ModuleNavBar.vue      # 底部模块切换导航栏
│           │   ├── FundEditModal.vue     # 基金持仓编辑弹窗
│           │   ├── FeeSettingModal.vue   # 股票手续费设置弹窗
│           │   ├── GoldTradeModal.vue    # 黄金加减仓弹窗
│           │   ├── GoldFeeSettingModal.vue # 黄金手续费设置弹窗
│           │   └── MarkdownRenderer.vue  # 更新日志 Markdown 渲染
│           ├── locales/    # 国际化语言文件 (zh / en / default)
│           ├── router/     # Vue 路由控制器
│           ├── composables/# 组合式函数 (更新检测等)
│           ├── utils/      # 通用工具函数
│           └── assets/     # 静态应用资源

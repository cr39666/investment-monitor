export default {
  // Setting.vue
  goBack: 'Back',
  about: 'About',
  settings: 'Settings',
  alwaysOnTop: 'Always On Top',
  topBall: '悬浮球',
  topWindow: '主窗口',
  hotkeyLabel: 'Display/Hide Window Hotkey',
  pressKeys: 'Press keys...',
  clickToSet: 'Click to set',
  language: 'Language',
  ballDisplayMode: 'Ball Amount',
  ballModeStock: '盈亏',
  ballModeGold: '金价',
  ballModeNone: '不显示',
  showModules: 'Module Display',
  moduleStock: '股票',
  moduleGold: '黄金',
  moduleFund: '基金',
  selectionRequired: '不能空选',
  fundFeeNotSupported: 'Fund fee setting is not supported yet',
  english: 'English',
  chinese: '中文',
  default: 'Mixed',
  appVersion: 'Version',
  autoLaunch: 'Auto Start',
  groupGeneral: 'General',
  groupDisplay: 'Display',
  groupOperation: 'Operations',
  columnOrderStock: '股票',
  columnOrderFund: '基金',
  columnOrderHintReorder: '拖拽调整列顺序',
  columnOrderHintStockSplit: '启用/关闭 拆分列',
  columnOrderSplitToggle: '点击移除此拆分副列',
  columnOrderModalStockTitle: '股票列自定义',
  columnOrderModalFundTitle: '基金列自定义',
  columnCustomize: 'Customize Columns',
  resetData: 'Reset Data',
  resetStockConfirm: '此操作会将【股票持仓、关注列表】全部数据清空，需要手动重新添加，确认重置？',
  resetGoldConfirm: '此操作会将【黄金持仓】全部数据清空，需要手动重新添加，确认重置？',
  resetFundConfirm: '此操作会将【基金列表】全部数据清空，需要手动重新添加，确认重置？',
  resetDataDone: '已重置{module}数据',
  columnOrderReset: '重置',
  columnOrderResetDone: '已重置为默认',
  colName: 'Name',
  colNav: 'NAV',
  colPnl: 'PnL',
  colChg: 'Chg',
  colYield: 'Yield',
  colLast: 'Days/Val',

  // About.vue
  investment: 'A-shares & Gold & Funds',
  monitor: 'investment monitor — designed by',
  designedBy: '',
  seeDetails: ', see details on ',
  clickToShrink: 'Click to shrink to ball',
  newVersionFound: '检测到新版本 v{version}，查看更新',
  newVersionFoundGeneric: '检测到新版本，查看更新',
  confirmUpdate: '下载更新',
  checkUpdates: 'Check for updates',
  viewChangelog: 'View changelog',
  checking: 'Checking...',
  newVersionAvailable: 'New version available',
  downloading: 'Downloading...',
  upToDate: 'Up to date',
  updateError: 'Update error,please try again',
  readyToInstall: 'Ready to install',
  install: 'Install',
  whatsNew: "更新内容 What's New",

  // MainList.vue
  stock: 'Stock',
  stockHolding: 'Stock-Holdings',
  stockWatch: 'Stock-Follows',
  name: '股票名称',
  code: 'Add Holding',
  currentPrice: '当前价',
  priceAndChange: '当前价/涨跌幅',
  dailyPnl: '当日盈亏',
  dailyPnlPercent: '当日盈亏比',
  totalPnl: '总盈亏',
  totalPnlPercent: '总盈亏比',
  avgBuyPrice: '均摊成本',
  marketValue: '持仓市值',
  change: '涨跌幅',
  changeAmount: '涨跌额',
  amount: '持仓手数',
  thName: 'Name',
  thCode: 'Code',
  stockCode: 'Stock Code',
  thPrice: 'Price',
  thChg: 'Chg%',
  thChangeAmount: 'Chg',
  thPriceChg: 'P/Chg%',
  thDPnl: 'D.PnL',
  thDPnlPct: 'D.PnL%',
  thDPnlBoth: 'D.P/D%',
  thTPnl: 'T.PnL',
  thTPnlPct: 'T.PnL%',
  thTPnlBoth: 'T.P/T%',
  thAvg: 'Avg',
  thVal: 'Val',
  thAvgVal: 'Avg/Val',
  thQty: 'Qty🖐️',
  thAlert: 'Alert🔔',
  labelD: 'D:',
  labelH: 'H:',
  noStocks: 'No Holdings',
  noWatchStocks: 'No Follows',
  watchStockCode: 'Add Follow',
  backToBall: '返回悬浮球',
  goToSetting: '设置',
  deleteSelected: '删除选中 Delete',
  clearAll: '清空列表 Clear',
  dailyPnlTotal: '当日盈亏合计 Daily Total',
  holdingPnlTotal: '总盈亏合计 Total PnL',
  clickToAdjust: '点击调整持仓',
  viewAdjustmentRecords: '查看当日调仓记录',
  stockExists: '该股票已存在',
  stockAdded: '股票已添加!',
  stockNotFound: '未找到该股票，请检查代码是否正确',
  watchStockExists: '该关注股票已存在',
  watchStockAdded: '关注股票已添加!',
  watchStockNotFound: '未找到该股票，请检查代码是否正确',
  selectedRemoved: '已删除选中股票',
  allCleared: '已清空所有股票',
  positionUpdated: '持仓已更新',
  adjustmentUndone: '已撤销最后一条调仓',
  adjustmentUndoConflict: '当前持仓已变化，无法安全撤销',
  amountCannotBeNegative: '持仓手数不能小于0',
  switchToWatchStocks: '切换到关注页',
  switchToHoldings: '切换到持仓页',
  clearWatchConfirm: '确认要清空关注列表所有数据吗？',

  // Price Alert
  priceAlert: '价格提醒',
  setPriceAlert: '设置价格提醒',
  priceAlertSet: '价格提醒已设置!',
  priceAlertUpCleared: '涨价提醒已取消',
  priceAlertDownCleared: '跌价提醒已取消',
  priceAlertTitle: 'Price Alert',
  targetPrice: '目标价格',
  alertUp: '涨到 Rise to',
  alertDown: '跌到 Fall to',
  clearAlert: '清除 Clear',
  priceAlertUpError: '目标价格必须高于当前价格',
  priceAlertDownError: '目标价格必须低于当前价格',

  // Modal.vue
  initialCost: '初始成本',
  initialCostTipTitle: '初始成本提示',
  initialCostTipMessage:
    '由于此处输入成本与“实际成本”小数位差异，故计算出的【总盈亏】结果可能会有偏差，【手数】越多偏差相应越大，望知悉',
  tradePrice: '💰成交价格',
  lotsHint: '手数(1手=100股)',
  deltaLots: '🖐️变动手数',
  addPosition: '添加持仓',
  adjustPosition: '调仓确认',
  tradeBuy: '买入',
  tradeSell: '卖出',
  selectTradeDirection: '请先选择买入/卖出/清仓',
  tradeAmountZero: '请输入手数',
  currentLots: '当前持有',
  lotsUnit: '手',
  clearPosition: '清仓',
  clearPositionTitle: '清仓确认',
  clearPositionConfirm: '以 {price} 卖出 {name} 的全部持仓 {amount} 手，预计手续费 ¥{fee}？',
  isTodayNewPosition: '是当日新建仓',
  isTodayTrade: '是当日操作',
  notTodayTrade: '非当日操作',
  estFee: '手续费',
  adjustmentRecordsTitle: '当日调仓记录',
  adjustmentRecordsEmpty: '当日暂无调仓记录',
  latestAdjustment: '最后一条',
  adjustmentHistoryTip: '仅保留当日记录',
  undoLastAdjustment: '撤销最后一条',
  undoAdjustmentTitle: '撤销调仓',
  undoAdjustmentConfirm: '是否要撤销最后一条调仓操作？',

  // Confirm.vue
  confirm: '确定',
  cancel: '取消',
  deleteStock: '删除股票',
  deleteConfirm: '确定要删除选中的 {count} 只股票吗？',
  clearList: '清空列表',
  clearConfirm: '确认要清空持仓列表所有数据吗？',

  // FloatingBall.vue
  dragToMove: '拖拽移动，左击展开，右击菜单',

  // GoldView.vue
  gold: 'Gold',
  metalGold: 'XAU',
  metalSilver: 'XAG',
  fetchError: '数据拉取失败，重试中...',
  grams: '按克数',
  enterGrams: '请输入克数',
  enterAmount: '请输入金额',
  estimatedCost: '按金额',
  feeHint: '手续费 {rate}',
  noGoldHolding: '暂无黄金持仓',
  invalidGrams: '请输入有效的克数',
  invalidAvgPrice: '请输入有效的成本价',
  noPriceData: '暂无价格数据',
  insufficientHolding: '持仓不足',
  buySuccess: '加仓成功',
  sellSuccess: '减仓成功',
  initSuccess: '持仓初始化成功',
  initHolding: '初始化持仓',
  holdingGrams: '持有克数',
  avgPrice: '均摊成本',
  valLabel: '价值',
  gramsLabel: '克数',
  avgCostPrice: '成本均价(¥/g)',
  tradePriceLabel: '成交价格(¥/g)',
  goldTotalPnl: '总盈亏',
  goldMarketValue: '持仓市值',
  goldYieldRate: '收益率',
  addPos: '加仓',
  reducePos: '减仓',
  clearPos: '清仓',
  clearGoldConfirm: '确认要清空黄金持仓所有数据吗？',
  viewGoldHolding: '查看黄金持仓',
  collapseGoldHolding: '收起黄金持仓',
  clickToTogglePnL: '点击切换盈亏显示',
  toggleCurrency: '切换货币',
  goldPriceNoticeTitle: '价格说明',
  goldPriceNotice: '显示价格为国际金/银价，与国内实时价格会有偏差，望知悉',

  // FundView.vue
  fund: 'Fund',
  thFundName: 'Name',
  thNav: 'NAV',
  thPnl: 'PnL',
  thFundChg: 'Chg%',
  thYield: 'Yield%',
  thDays: 'Days',
  thFundVal: 'Val',
  labelTPnl: 'T.PnL:',

  noFunds: 'No Funds',
  fundCode: 'Fund Code',
  fundCodeInvalid: '请输入6位基金代码',
  fundExists: '该基金已存在',
  fundAdded: '基金已添加!',
  fundNotFound: '未找到该基金，请检查代码是否正确',
  fundNav: '最新净值',
  deleteFund: '删除基金',
  deleteFundConfirm: '确定要删除选中的 {count} 只基金吗？',
  clearFundConfirm: '确认要清空基金列表所有数据吗？',
  addFundPosition: '添加基金持仓',
  editFundPosition: '编辑持仓',
  fundCostNav: '成本净值',
  fundShares: '持有份额',
  clickToEdit: '点击编辑持仓',
  holdingDays: '持有天数',
  yieldRate: '收益率',
  fundBuyDate: '买入日期',

  // FeeSettingModal
  feeSetting: 'Fee Settings',
  feeSettingTitle: 'A股手续费配置',
  feeSaved: '手续费配置已保存',
  feeCommissionRate: '佣金费率',
  feeCommissionRateUnit: '万分之',
  feeMinCommission: '最低佣金',
  feeMinCommissionUnit: '元',
  feeTransferRate: '过户费费率',
  feeTransferRateUnit: '万分之',
  feeStampTaxRate: '印花税费率',
  feeStampTaxRateUnit: '万分之',
  feeBuyFormula: '买入手续费 = 佣金 + 过户费',
  feeSellFormula: '卖出手续费 = 佣金 + 印花税 + 过户费',

  // GoldFeeSettingModal
  goldFeeSettingTitle: '黄金手续费配置',
  goldFeeSaved: '黄金手续费配置已保存',
  goldBuyFeeRate: '买入费率',
  goldSellFeeRate: '卖出费率',
  goldBuyFeeHint: '买入时收取，默认0%',
  goldSellFeeHint: '卖出时收取，默认0.4%',
  goldBuyFeeFormula: '买入成本 = 金额 + 金额 × 买入费率',
  goldSellFeeFormula: '卖出扣除 = 金额 × 卖出费率',

  // 黄金交易二次确认
  goldBuyAction: '买入',
  goldSellAction: '卖出',
  goldTradeAmount: '金额',
  goldTradeConfirmTitle: '确认交易'
}

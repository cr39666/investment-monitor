export default {
  // Setting.vue
  goBack: 'Back',
  about: 'About',
  settings: 'Settings',
  alwaysOnTop: 'Always On Top',
  topBall: 'Ball',
  topWindow: 'MainWindow',
  hotkeyLabel: 'Display/Hide Window Hotkey',
  pressKeys: 'Press keys...',
  clickToSet: 'Click to set',
  language: 'Language',
  ballDisplayMode: 'Ball Amount',
  ballModeStock: 'Stock PnL',
  ballModeGold: 'Gold Price',
  ballModeNone: 'Hide',
  showModules: 'Module Display',
  moduleStock: 'Stock',
  moduleGold: 'Gold',
  moduleFund: 'Fund',
  selectionRequired: 'Selection Required',
  fundFeeNotSupported: 'Fund fee setting is not supported yet',
  english: 'English',
  chinese: '中文',
  default: 'Mixed',
  appVersion: 'Version',
  autoLaunch: 'Auto Start',
  groupGeneral: 'General',
  groupDisplay: 'Display',
  groupOperation: 'Operations',
  columnOrderStock: 'Stock',
  columnOrderFund: 'Fund',
  columnOrderHintReorder: 'Drag to reorder columns',
  columnOrderHintStockSplit: 'Enable / Disable split columns',
  columnOrderSplitToggle: 'Click to remove this split column',
  columnOrderModalStockTitle: 'Stock Columns',
  columnOrderModalFundTitle: 'Fund Columns',
  columnCustomize: 'Customize Columns',
  resetData: 'Reset Data',
  resetStockConfirm:
    'This operation will clear all [Stock Holdings, Follows List] data. You need to manually re-add them. Confirm reset?',
  resetGoldConfirm:
    'This operation will clear all [Gold Holdings] data. You need to manually re-add them. Confirm reset?',
  resetFundConfirm:
    'This operation will clear all [Fund List] data. You need to manually re-add them. Confirm reset?',
  resetDataDone: '{module} data reset',
  columnOrderReset: 'Reset',
  columnOrderResetDone: 'Restored default order',
  // Column names (labels in fund "column order" setting)
  colName: 'Name',
  colNav: 'NAV',
  colPnl: 'PnL',
  colChg: 'Chg',
  colYield: 'Yield',
  colLast: 'Days/Val',

  // About.vue
  investment: 'A-shares & Gold & Funds',
  monitor: 'investment monitor — designed by',
  designedBy: '',
  seeDetails: ', see details on ',
  clickToShrink: 'Click to shrink to ball',
  newVersionFound: 'New version v{version} detected, view update',
  newVersionFoundGeneric: 'New version detected, view update',
  confirmUpdate: 'Download Update',
  checkUpdates: 'Check for updates',
  viewChangelog: 'View changelog',
  checking: 'Checking...',
  newVersionAvailable: 'New version available',
  downloading: 'Downloading...',
  upToDate: 'Up to date',
  updateError: 'Update error,please try again',
  readyToInstall: 'Ready to install',
  install: 'Install',
  whatsNew: "What's New",

  // MainList.vue
  stock: 'Stock',
  stockHolding: 'Stock-Holdings',
  stockWatch: 'Stock-Follows',
  name: 'Stock Name',
  code: 'Add Holding',
  currentPrice: 'Current Price',
  priceAndChange: 'Price / Change',
  dailyPnl: 'Daily PnL',
  dailyPnlPercent: 'Daily PnL %',
  totalPnl: 'Total PnL',
  totalPnlPercent: 'Total PnL %',
  avgBuyPrice: 'Avg. Buy Price',
  marketValue: 'Market Value',
  change: 'Change',
  changeAmount: 'Change Amount',
  amount: 'Amount',

  // Table headers
  thName: 'Name',
  thCode: 'Code',
  stockCode: 'Stock Code',
  thPrice: 'Price',
  thChg: 'Chg%',
  thChangeAmount: 'Chg',
  thPriceChg: 'P/Chg%',
  thDPnl: 'D.PnL',
  thDPnlPct: 'D.PnL%',
  thDPnlBoth: 'D.P/D%',
  thTPnl: 'T.PnL',
  thTPnlPct: 'T.PnL%',
  thTPnlBoth: 'T.P/T%',
  thAvg: 'Avg',
  thVal: 'Val',
  thAvgVal: 'Avg/Val',
  thQty: 'Qty🖐️',
  thAlert: 'Alert🔔',
  labelD: 'D:',
  labelH: 'H:',
  noStocks: 'No Holdings',
  noWatchStocks: 'No Follows',
  watchStockCode: 'Add Follow',
  backToBall: 'Back to ball',
  goToSetting: 'settings',
  deleteSelected: 'Delete Selected',
  clearAll: 'Clear All',
  dailyPnlTotal: 'Daily PnL Total',
  holdingPnlTotal: 'Total PnL',
  clickToAdjust: 'Click to adjust quantity',
  viewAdjustmentRecords: "View today's adjustment records",
  stockExists: 'Stock already exists',
  stockAdded: 'Stock Added!',
  stockNotFound: 'Stock not found, please check the code',
  watchStockExists: 'Follow stock already exists',
  watchStockAdded: 'Follow Stock Added!',
  watchStockNotFound: 'Stock not found, please check the code',
  selectedRemoved: 'Selected Stocks Removed',
  allCleared: 'All Stocks Cleared',
  positionUpdated: 'Position Updated',
  adjustmentUndone: 'Last adjustment undone',
  adjustmentUndoConflict: 'Position changed; undo is not safe',
  amountCannotBeNegative: 'Position cannot be negative',
  switchToWatchStocks: 'Switch to Followlist',
  switchToHoldings: 'Switch to Holdings',
  clearWatchConfirm: 'Are you sure you want to clear all follow list data?',

  // Price Alert
  priceAlert: 'Price Alert',
  setPriceAlert: 'Set Price Alert',
  priceAlertSet: 'Price Alert Set!',
  priceAlertUpCleared: 'Rise Alert Cleared',
  priceAlertDownCleared: 'Fall Alert Cleared',
  priceAlertTitle: 'Price Alert',
  targetPrice: 'Target Price',
  alertUp: 'Rise to',
  alertDown: 'Fall to',
  clearAlert: 'Clear',
  priceAlertUpError: 'Target price must be higher than current price',
  priceAlertDownError: 'Target price must be lower than current price',

  // Modal.vue
  initialCost: 'Initial Cost',
  initialCostTipTitle: 'Initial Cost Tip',
  initialCostTipMessage:
    'Decimal differences from the actual cost may affect Total PnL. More lots may cause larger deviations.',
  tradePrice: '💰Trade Price',
  lotsHint: 'Lots (1 lot = 100 shares)',
  deltaLots: '🖐️Delta Lots',
  addPosition: 'Add Position',
  adjustPosition: 'Adjust Position',
  tradeBuy: 'Buy',
  tradeSell: 'Sell',
  selectTradeDirection: 'Choose Buy, Sell, or Sell All first',
  tradeAmountZero: 'Please enter lots',
  currentLots: 'Holding',
  lotsUnit: 'lot(s)',
  clearPosition: 'Sell All',
  clearPositionTitle: 'Sell All Confirmation',
  clearPositionConfirm: 'Sell all {amount} lot(s) of {name} at {price}, est. fee ¥{fee}?',
  isTodayNewPosition: "Today's New Position",
  isTodayTrade: "Today's Trade",
  notTodayTrade: 'Not Today',
  estFee: 'Fee',
  adjustmentRecordsTitle: "Today's Adjustments",
  adjustmentRecordsEmpty: 'No adjustment records today',
  latestAdjustment: 'Latest',
  adjustmentHistoryTip: 'Only today is kept',
  undoLastAdjustment: 'Undo Latest',
  undoAdjustmentTitle: 'Undo Adjustment',
  undoAdjustmentConfirm: 'Undo the latest adjustment?',

  // Confirm.vue
  confirm: 'Confirm',
  cancel: 'Cancel',
  deleteStock: 'Delete Stock',
  deleteConfirm: 'Are you sure you want to delete {count} selected stock(s)?',
  clearList: 'Clear List',
  clearConfirm: 'Are you sure you want to clear all holding data?',

  // FloatingBall.vue
  dragToMove: 'Drag to move, left click to expand, right click for menu',

  // GoldView.vue
  gold: 'Gold',
  metalGold: 'Gold',
  metalSilver: 'Silver',
  fetchError: 'Fetch failed, retrying...',
  grams: 'By Grams',
  enterGrams: 'Enter grams',
  enterAmount: 'Enter amount',
  estimatedCost: 'By Amount',
  feeHint: 'Fee {rate}',
  noGoldHolding: 'No gold holding',
  invalidGrams: 'Please enter valid grams',
  invalidAvgPrice: 'Please enter a valid average price',
  noPriceData: 'No price data',
  insufficientHolding: 'Insufficient holding',
  buySuccess: 'Add successful',
  sellSuccess: 'Reduce successful',
  initSuccess: 'Holding initialized',
  initHolding: 'Initialize Holding',
  holdingGrams: 'Holding Grams',
  avgPrice: 'Average Price',
  valLabel: 'Value',
  gramsLabel: 'Grams',
  avgCostPrice: 'Avg Cost (¥/g)',
  tradePriceLabel: 'Trade Price (¥/g)',
  goldTotalPnl: 'Total PnL',
  goldMarketValue: 'Market Value',
  goldYieldRate: 'Yield Rate',
  addPos: 'Add',
  reducePos: 'Reduce',
  clearPos: 'Clear',
  clearGoldConfirm: 'Are you sure you want to clear all gold holding data?',
  viewGoldHolding: 'View Gold Holding',
  collapseGoldHolding: 'Collapse Gold Holding',
  clickToTogglePnL: 'Click to toggle PnL display',
  toggleCurrency: 'Toggle Currency',
  goldPriceNoticeTitle: 'Price Note',
  goldPriceNotice:
    'Displayed prices are international gold/silver prices and may differ from domestic real-time prices.',

  // FundView.vue
  fund: 'Fund',
  thFundName: 'Name',
  thNav: 'NAV',
  thPnl: 'PnL',
  thFundChg: 'Chg%',
  thYield: 'Yield%',
  thDays: 'Days',
  thFundVal: 'Val',
  labelTPnl: 'T.PnL:',

  noFunds: 'No funds',
  fundCode: 'Fund Code',
  fundCodeInvalid: 'Please enter a 6-digit fund code',
  fundExists: 'Fund already exists',
  fundAdded: 'Fund Added!',
  fundNotFound: 'Fund not found, please check the code',
  fundNav: 'Net Asset Value',
  deleteFund: 'Delete Fund',
  deleteFundConfirm: 'Are you sure you want to delete {count} selected fund(s)?',
  clearFundConfirm: 'Are you sure you want to clear all fund data?',
  addFundPosition: 'Add Fund Position',
  editFundPosition: 'Edit Position',
  fundCostNav: 'Cost NAV',
  fundShares: 'Shares',
  clickToEdit: 'Click to edit position',
  holdingDays: 'Holding Days',
  yieldRate: 'Yield Rate',
  fundBuyDate: 'Buy Date',

  // FeeSettingModal
  feeSetting: 'Fee Settings',
  feeSettingTitle: 'Stock Fee Config',
  feeSaved: 'Fee config saved',
  feeCommissionRate: 'Commission Rate',
  feeCommissionRateUnit: '‱ (per 10k)',
  feeMinCommission: 'Min Commission',
  feeMinCommissionUnit: 'CNY',
  feeTransferRate: 'Transfer Fee',
  feeTransferRateUnit: '‱ (per 10k)',
  feeStampTaxRate: 'Stamp Tax',
  feeStampTaxRateUnit: '‱ (per 10k)',
  feeBuyFormula: 'Buy = Commission + Transfer Fee',
  feeSellFormula: 'Sell = Commission + Stamp Tax + Transfer Fee',

  // GoldFeeSettingModal
  goldFeeSettingTitle: 'Gold Fee Config',
  goldFeeSaved: 'Gold fee config saved',
  goldBuyFeeRate: 'Buy Fee Rate',
  goldSellFeeRate: 'Sell Fee Rate',
  goldBuyFeeHint: 'Charged on buy, default 0%',
  goldSellFeeHint: 'Charged on sell, default 0.4%',
  goldBuyFeeFormula: 'Buy Cost = Amount + Amount × Buy Rate',
  goldSellFeeFormula: 'Sell Deduction = Amount × Sell Rate',

  // Gold trade confirmation
  goldBuyAction: 'Buy',
  goldSellAction: 'Sell',
  goldTradeAmount: 'Amount',
  goldTradeConfirmTitle: 'Confirm Trade'
}
